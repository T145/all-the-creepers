		...
		for (int x = -radius; x <= radius; x++) {
			for (int y = -radius; y <= radius; y++) {
				for (int z = -radius; z <= radius; z++) {
					...
					if (state.getBlock().canPlaceBlockAt(world, MUTABLE_POS)
		...
		for (int x = -radius; x <= radius; x++) {
			for (int y = -radius; y <= radius; y++) {
				for (int z = -radius; z <= radius; z++) {
					...
					if (block.canPlaceBlockAt(world, MUTABLE_POS)
							&& !block.canPlaceBlockAt(world, new BlockPos(posX + x, posY + y - 1, posZ + z))
							&& rand.nextBoolean()) {
						world.setBlockState(MUTABLE_POS, state, ModConfig.GENERAL.updatePlacedBlocks ? 3 : 2);
					}
				}
			...
			for (BlockPos.MutableBlockPos mutablePos1 : BlockPos.getAllInBoxMutable(pos.add(-f, -1.0D, -f), pos.add(f, -1.0D, f))) {
				if (mutablePos1.distanceSqToCenter(entity.posX, entity.posY, entity.posZ) <= (f * f)) {
					...
					if (iblockstate.getMaterial() == Material.AIR) {
						...
						if (iblockstate1.getMaterial() == liquid.getDefaultState().getMaterial()
								&& (iblockstate1.getBlock() == liquid || iblockstate1.getBlock() == flowingLiquid)
								&& iblockstate1.getValue(BlockLiquid.LEVEL) == 0
								&& world.mayPlace(platform, mutablePos1, false, EnumFacing.DOWN, null)) {
